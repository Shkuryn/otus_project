...
Процедура ИсполняемыеСценарии() Экспорт 
	...
	.ДобавитьТест("ТестСозданиеДисконтнойКартыПозитив", "Создание дисконтной карты", "Позитив, Шкурин Игорь, Тест №1") 
	.ДобавитьТест("ТестСозданиеДисконтнойКартыНегатив", "Создание дисконтной карты", "Негатив, Шкурин Игорь, Тест №1") 
...
Процедура ТестСозданиеДисконтнойКартыПозитив() Экспорт   
	
	//1. подготовка параметров и генерация данных для выполнения функции
	КонтрагентНаименование = "Иванов Игорь Леопардович";
	Контрагент = СозданиеКонтрагента(КонтрагентНаименование);
	
	Конструктор = ЮТест.Данные().КонструкторОбъекта("Справочник.ДисконтныеКарты")
	.Установить("Код", "000000001")
	.Установить("Владелец", Контрагент);
	//2. проверка результата
	ЮТест.ОжидаетЧто(Конструктор).Метод("Записать").НеВыбрасываетИсключение();		
КонецПроцедуры      

Процедура ТестСозданиеДисконтнойКартыНегатив() Экспорт   
	...
	КодКарты = "000000001";
	ПолеКод = "Код";
	КонструкторДК1 = ЮТест.Данные().КонструкторОбъекта("Справочник.ДисконтныеКарты")
	.Установить("Код", КодКарты)
	.Установить("Владелец", Контрагент).Записать();
	
	КонструкторДК2 = ЮТест.Данные().КонструкторОбъекта("Справочник.ДисконтныеКарты")
	.Установить("Код", КодКарты)
	.Установить("Владелец", Контрагент);
	//2. проверка результата                                                                         
	
	//ТекстОшибки = СтрШаблон(НСтр("ru='Значение %КодКарты поля %ПолеКод не уникально'", "ru"), КодКарты, ПолеКод);  
	ТекстОшибки = СтрШаблон("Значение `%1` поля `%2` не уникально", КодКарты, ПолеКод);
	ТекстОшибки = СтрЗаменить(ТекстОшибки, "`", """");
	
	ЮТест.ОжидаетЧто(КонструкторДК2).Метод("Записать").ВыбрасываетИсключение(ТекстОшибки);		
